...
from kognic.auth._user_agent import (
    _PY_VERSION,  # pyright: ignore[reportPrivateUsage]
    get_user_agent,
)
...
class TestGetUserAgent(unittest.TestCase):
    ...
    def test_user_agent_with_client_name(self) -> None:
    ...
    def test_user_agent_with_none_client_name(self) -> None:
        ua = get_user_agent("requests/2.31.0", None)
        self.assertEqual(ua, f"kognic-auth/{__version__} python/{_PY_VERSION} requests/2.31.0")

    def test_user_agent_with_empty_client_name(self) -> None:
        ua = get_user_agent("requests/2.31.0", "")
